...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c  Powered by 495FD %c 你正在访问495FDのBlog",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("495FD") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('495FD的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">退吧</span>`
      } else if (fps <= 15) {
        var kd = `<span style="color:red">玩不了</span>`
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">有点难玩</span>`
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">坚持</span>`
      } else if (fps <= 45) {
        var kd = `<span style="color:#08b7e4">还可以</span>`
      } else {
        var kd = `<span style="color:#39c5bb">流畅的一</span>`
      }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://s2.loli.net/2022/11/04/nzEBk21ebfZ4tJA.png);
    --darkmode-bg:url(https://s2.loli.net/2022/11/04/nzEBk21ebfZ4tJA.png);
    --mobileday-bg: url(https://s2.loli.net/2022/11/04/nzEBk21ebfZ4tJA.png);
    --mobilenight-bg: url(https://s2.loli.net/2022/11/04/nzEBk21ebfZ4tJA.png);
  }`;
}
// 切换背景主函数
function changeBg(s) {
